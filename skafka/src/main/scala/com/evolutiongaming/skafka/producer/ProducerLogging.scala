...
object ProducerLogging {
  ...
  def apply1[F[_] : MonadThrow : ch.MeasureDuration](producer: Producer[F], log: Log[F]): Producer[F] =
  ...
  /**
    * @param charsToTrim a number of chars from record's value to log when producing fails because of a too large record
    */
  def apply1[F[_]: MonadThrow: ch.MeasureDuration](producer: Producer[F], log: Log[F], charsToTrim: Int): Producer[F] = {
