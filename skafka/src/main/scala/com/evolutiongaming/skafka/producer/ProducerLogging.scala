...
object ProducerLogging {
  ...
  /**
    ...
    */
  @deprecated("Use `apply1` instead", "15.2.0")
  def apply[F[_]: MonadThrow: smetrics.MeasureDuration](producer: Producer[F], log: Log[F], charsToTrim: Int): Producer[F] = {
    implicit val md: MeasureDuration[F] = smetrics.MeasureDuration[F].toCatsHelper
  ...
  /**
    * @param charsToTrim a number of chars from record's value to log when producing fails because of a too large record
    */
  def apply1[F[_]: MonadThrow: MeasureDuration](producer: Producer[F], log: Log[F], charsToTrim: Int): Producer[F] = {
